...
$(() => {
    ...
    toycash_contract = new web3js.eth.Contract(toycashContractABI, toycashContractAddress);
    blockdoubt_contract = new web3js.eth.Contract(blockdoubtContractABI, blockDoubtContractAddress);
    methods = contract.methods;
    createCommand();
});

console.log(`イベントの取得方法
await contract.getPastEvents("Transfer", {
    filter: {_to: "0x2093bf5d91568C466b6e989eC4F441d90fA1c048"},
    fromBlock: 0,
    toBlock: 'latest'
}, function(error, events){
    if (!error) {
        console.log("returnValues", events.map(v => {
            const w = v.returnValues;
            return {
                "_from": w._from,
                "_to": w._to,
                "_tokenId": w._tokenId
