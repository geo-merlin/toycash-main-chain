def register_investment(amount, token, screen_name, message):
    #パトロンからの出資内容(出資額と出資するトークンの種類、出資先のツイッターアカウント)をDynamoDBに登録する処理を担う関数
    pass
...
def handle_FMresponse(res):
    #FMから来た返答(半額、全額、見ないフリ)をDynamoDBに登録する処理を担う関数
    pass

def confirm_distribution(screen_name_list):
    #パトロンからの出資について配分が確定したらその通りに残高を操作しDynamoDBに記録する処理を担う関数
    pass
...
def main():
    ...
    1.サーバーは我々のアカウントにTipがあったことを察知してTipツイートから内容を読み取ってDynamoDBに登録する :register_investment
    ...
    3.返信に応じてパトロンからの出資を受け取る処理を行う :handle_FMresponse
        a.半額or見ないふりだった場合
            配分をDynamoDBに記録して確定させる :confirm_distribution
